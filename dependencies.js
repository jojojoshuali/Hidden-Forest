...
window.ShadowShader = window.classes.ShadowShader =
class ShadowShader extends Shader         // Simple "procedural" texture shader, with texture coordinates but without an input image.
{ material() { return { shader: this } }  // Materials here are minimal, without any settings.
  map_attribute_name_to_buffer_name( name )                  // We'll pull single entries out per vertex by field name.  Map
    {                                                        // those names onto the vertex array names we'll pull them from.
      return { object_space_pos: "positions", tex_coord: "texture_coords" }[ name ]; }      // Use a simple lookup table.
  ...
  vertex_glsl_code()           // ********* VERTEX SHADER *********
    { return `
        ...
        void main() {
          ...
          gl_Position = u_PVM_transform * vec4(a_Vertex, 1.0);                                      // Directly use original texture coords and interpolate between.
        }`;
    }
  fragment_glsl_code()           // ********* FRAGMENT SHADER *********
    { return `
        // high precision for int, vertex positions
        precision mediump int;
        // low precision for float, texture positions
...
window.Shape2 = window.tiny_graphics.Shape2 =
class Shape2 extends Vertex_Buffer
{
  static insert_transformed_copy_into( recipient, args, points_transform = Mat4.identity() )    // For building compound shapes.
    { const temp_shape = new this( ...args );  // If you try to bypass making a temporary shape and instead directly insert new data into
      ...
      { if( a == "positions" )                          // Apply points_transform to all points added during this call:
          ...
          recipient[a].push( ...temp_shape[a].map( t => Mat4.inverse( points_transform.transposed() ).times( t.to4(1) ).to3() ) );
        else if (a == "bitangents")
          recipient[a].push( ...temp_shape[a].map( b => Mat4.inverse( points_transform.transposed() ).times( b.to4(1) ).to3() ) );
...
class Square2 extends Shape2              // A square, demonstrating two triangles that share vertices.  On any planar surface, the interior
                                        // edges don't make any important seams.  In these cases there's no reason not to re-use data of
{                                       // the common vertices between triangles.  This makes all the vertex arrays (position, normals,
  constructor()                         // etc) smaller and more cache friendly.
...
class Cube2 extends Shape2    // A cube inserts six square strips into its arrays.
{ constructor()
    { super( "positions", "normals", "tangents", "bitangents", "texture_coords" );
      for( var i = 0; i < 3; i++ )
        for( var j = 0; j < 2; j++ )
...
class Bump_Shader extends Phong_Shader          // THE DEFAULT SHADER: This uses the Phong Reflection Model, with optional Gouraud shading.
                                           // Wikipedia has good defintions for these concepts.  Subclasses of class Shader each store
                                           // and manage a complete GPU program.  This particular one is a big "master shader" meant to
                                           // handle all sorts of lighting situations in a configurable way.
                                           // Phong Shading is the act of determining brightness of pixels via vector math.  It compares
...
{ material( color, properties )     // Define an internal class "Material" that stores the standard settings found in Phong lighting.
  ...
  map_attribute_name_to_buffer_name( name )                  // We'll pull single entries out per vertex by field name.  Map
    {                                                        // those names onto the vertex array names we'll pull them from.
      return { object_space_pos: "positions", normal: "normals", tangent: "tangents", bitangent: "bitangents", tex_coord: "texture_coords" }[ name ]; }   // Use a simple lookup table.
  shared_glsl_code()            // ********* SHARED CODE, INCLUDED IN BOTH SHADERS *********
    { return `
        precision highp float;
        const int N_LIGHTS = 2;             // We're limited to only so many inputs in hardware.  Lights are costly (lots of sub-values).
  ...
  vertex_glsl_code()           // ********* VERTEX SHADER *********
    { return `
        ...
        void main()
        {
          ...
          ts_frag_pos = vec3(camera_model_transform * vec4(object_space_pos, 1.0));

          vec3 T = normalize( inverse_transpose_modelview * tangent );
          vec3 B = normalize( inverse_transpose_modelview * bitangent );
          vec3 N = normalize( inverse_transpose_modelview * normal );                             // The final normal vector in screen space.
          mat3 TBN = transpose(mat3(T,B,N));

          ts_light_pos = TBN * lightPosition[0].xyz;
          ts_view_pos = TBN * vec3(0, 0, 5);
  ...
  fragment_glsl_code()           // ********* FRAGMENT SHADER *********
    {                            // A fragment is a pixel that's overlapped by the current triangle.
      ...
      return `
        uniform sampler2D texture;
        uniform sampler2D texture_norm;     // added for bump mapping
        ...
        { if( GOURAUD || COLOR_NORMALS )    // Do smooth "Phong" shading unless options like "Gouraud mode" are wanted instead.
          ...
          if( USE_TEXTURE ) {
            vec3 norm = normalize( texture2D(texture_norm, f_tex_coord).rgb * 2.0 - 1.0 );
            float diffuse = max( dot(light_dir, norm), 0.0 );
            gl_FragColor = vec4(diffuse * tex_color.xyz + ambient * tex_color.xyz, 1.0);
  ...
  update_GPU( g_state, model_transform, material, gpu = this.g_addrs, gl = this.gl )
    {                              // First, send the matrices to the GPU, additionally cache-ing some products of them we know we'll need:
      ...
      { gpu.shader_attributes["tex_coord"].enabled = true;
        ...
        gl.bindTexture(gl.TEXTURE_2D, material.texture[0].id);
        gl.uniform1i(gpu.texture_norm_loc, 0);

        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture( gl.TEXTURE_2D, material.texture[1].id );
  ...
  constructor( context, control_box )             // The scene begins by requesting the camera, shapes, and materials it will need.
    { super( context, control_box );              // First, include a couple other helpful components, including one that moves you around:
      ...
      const shapes = { 'triangle'        : new Triangle(),                // At the beginning of our program, load one of each of these shape 
                       ...
                       'box'             : new Cube(),
                       'bump_box'        : new Cube2(),            // multiple cubes.  Don't define more than one blueprint for the
